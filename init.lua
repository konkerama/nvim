...
require("lazy").setup({
	...
	{ -- Autoformat
		...
		opts = {
			...
			-- NOTE: format_on_save lives in lua/custom/plugins/conform.lua (it raises the
			-- timeout to 4000ms for slow terraform formatters). lazy.nvim merges opts, but
			-- a second format_on_save here would just be overridden, so it is intentionally absent.
			formatters_by_ft = {
				lua = { "stylua" },
